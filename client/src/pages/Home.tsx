...
export default function Home() {
  ...
  const featuredBlog = [...(blogPostsData as BlogPostPreview[])]
  ...
  return (
    <Layout showHero={true} currentPage="home">
      ...
      {featuredBlog && (
        <section id="featured-blog" className="py-20">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold mb-10 text-center">
              <span className="bg-gradient-to-r from-purple-500 to-purple-700 dark:from-purple-400 dark:to-purple-600 text-transparent bg-clip-text">Featured Blog</span>
            </h2>

            <div className="max-w-4xl mx-auto bg-white/90 dark:bg-black/40 rounded-2xl p-8 shadow-lg border border-purple-100 dark:border-purple-900/40">
              <div className="mb-4 flex items-center justify-between flex-wrap gap-3 text-sm text-gray-600 dark:text-gray-400">
                <span className="inline-block px-3 py-1 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 font-medium">
                  {featuredBlog.category}
                </span>
                <span>{format(new Date(featuredBlog.date), 'MMM d, yyyy')} • {featuredBlog.readTime}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {featuredBlog.tags.slice(0, 4).map((tag) => (
                  <span key={tag} className="px-2 py-1 rounded text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">
                    #{tag}
                  </span>
                ))}
              </div>
              ...
              <Link
                ...
                className="inline-flex items-center px-5 py-2.5 rounded-full bg-purple-600 hover:bg-purple-700 text-white font-medium transition-colors"
              >
                Read Featured Post
              </Link>
            </div>
